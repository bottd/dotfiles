		...
		(vim.api.nvim_create_autocmd [ "BufNewFile" "BufReadPost" ] {
			...
			:callback  (fn []
				(vim.schedule ()
					...
					(local is_empty (or ( (length lines 0)) (and ( (length lines) 1) ( (. lines 1) ""))))
					(when (and is_empty ( vim.bo.buftype ""))
            ...
            (cond
              ( (vim.fn.filereadable template_path) 1)
		...
		:core.dirman  {
			:config  {
				:workspaces  ((fn []
					...
					(each [_ ws (ipairs workspaces)]
						(tset config ws (.. workspace_path "/" ws)))
					config)))
...
(wk.add [
	{ 1 "<leader>nj" 2 ":Neorg journal<Cr>" :desc "Journal" :icon " " }
	{ 1 "<leader>n<space>" 2 ":Neorg index<Cr>" :desc "Index" :icon " " }
	{ 1 "<leader>na" :desc "Archive" :icon  " " }
	{ 1 "<leader>naf" 2 ":Neorg archive archive-file<Cr>" :desc "Archive file" :icon " " }
	{ 1 "<leader>nar" 2 ":Neorg archive restore-file<Cr>" :desc "Restore file" :icon " " }
	{ 1 "<leader>nt" 2 ":Neorg tangle<Cr>" :desc "tangle" }
	{ 1 "<leader>ne" :desc "Export" }
	{ 1 "<leader>nef" 2 ":Neorg export to-file<Cr>" :desc "Export file" }
	{ 1 "<leader>ned" 2 ":Neorg export directory<Cr>" :desc "Export directory" }
	{ 1 "<leader>nep" 2 (.. ":Neorg export directory " workspace_path "/public md<Cr>") :desc "Export posts" }
