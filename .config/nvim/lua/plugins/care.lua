...
local function _2_(entry, data)
  ...
  local _local_5_ = entry
  local completion_item = _local_5_["completion_item"]
  local _local_6_ = ((require("care.config")).options).ui
  local type_icons = _local_6_["type_icons"]
  local entry_kind
...
local function _8_(body)
  local _local_9_ = require("luasnip")
  local lsp_expand = _local_9_["lsp_expand"]
  return lsp_expand(body)
end
setup({ui = {ghost_text = {position = "inline"}, menu = {max_height = 10, format_entry = _2_}}, alignment = {"left", "right"}, selection_behavior = "insert", confirm_behavior = "replace", sorting_direction = "away-from-cursor", sources = {lsp = {max_entries = 5, priority = 1}, cmp_buffer = {max_entries = 3}}, snippet_expansion = _8_})
